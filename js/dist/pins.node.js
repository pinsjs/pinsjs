...
var pinRegistryRemove = function (name, board, unlink) {
  ...
  if (unlink) { dir.remove(removePath, { recursive: true }); }
...
var pinFind = function (text, ref) {
  ...
  for (var boardIdx in board) {
    ...
    try {
      boardPins = boardPinFind(
        boardObject,
        text,
        Object.assign.apply(Object, [ { name: name, extended: extended } ].concat( args ))
      );
    } catch (error) {
    ...
    if (boardPins.length > 0) {
      boardPins = dfCBind(
        boardPins,
        dataFrame(
          boardPins.map(function (e) { return Object.assign(e, { board: boardName }); }),
          { board: 'character' }
        )
      );
  ...
  if (!isNull(name)) {
    ...
    if (allPins.length > 0) { allPins = allPins.filter(function (e, idx) { return idx === 0; }); }
  }
